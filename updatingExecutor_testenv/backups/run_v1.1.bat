...
if "%DEBUG_MODE%"=="1" (
    echo.
    echo [DEBUG] Listing all items in workspace (will preserve backups and run.bat):
    echo.
    for /r "%WORKDIR%" %%F in (*) do (
        if not "%%~nxF"=="run.bat" if not "%%~dpF"=="%WORKDIR%backups" (
            echo [DEBUG] FILE: %%~nxF
        )
    )
    for /d %%D in ("%WORKDIR%*") do (
        if not "%%~nxD"=="backups" if not "%%~nxD"=="run_space" (
            echo [DEBUG] DIR: %%~nxD
        )
    )
    echo.
...
for %%F in ("%WORKDIR%*.*") do (
    if not "%%~nxF"=="run.bat" (
        del "%%F" >nul 2>&1
        if "%DEBUG_MODE%"=="1" echo [OK] %%~nxF deleted.
    )
)

REM Delete all subdirectories except backups
for /d %%D in ("%WORKDIR%*") do (
    if not "%%~nxD"=="backups" if not "%%~nxD"=="run_space" (
        rmdir /s /q "%%D" >nul 2>&1
        if "%DEBUG_MODE%"=="1" echo [OK] %%~nxD deleted.
    )
)

echo.
echo [INFO] Workspace wiped. Preserved: run.bat, backups/, run_space/
echo [INFO] Exiting.
timeout /t 2 >nul
goto :END
