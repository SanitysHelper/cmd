...
function Log-MenuFrame {
    ...
    Rotate-LogIfNeeded -Path $path -LimitBytes $script:paths.logLimitBytes
    $pathInfo = if ($CurrentPath) { " Path=$CurrentPath" } else { "" }
    "[$(Get-Timestamp)] MENU FRAME Selected=$SelectedIndex Count=$($Items.Count)$pathInfo" | Add-Content -Path $path -Encoding UTF8
